...
def decide(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if use_bedrock:
        ...
        model = os.environ.get("SUSPECTRA_LLM_MODEL") or os.environ.get(
        ...
        model = os.environ.get("SUSPECTRA_LLM_MODEL", DEFAULT_DIRECT_MODEL)
...
def _system_prompt() -> str:
    path = os.environ.get("SUSPECTRA_LLM_SYSTEM_PROMPT_PATH")
    if path:
        return Path(path).read_text(encoding="utf-8")
    return (
        "You control one Suspectra Crewrift player during a meeting. "
...
def _min_submit_confidence() -> float:
    return float(
        os.environ.get(
            "SUSPECTRA_LLM_MIN_SUBMIT_CONFIDENCE",
