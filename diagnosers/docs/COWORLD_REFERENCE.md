...
> Primary navigation guide for coding agents working in this `diagnosers` project. This is not the authoritative coworld spec - it is an index. Treat the metta sources as the source of truth.

## 1. What this project is

`diagnosers` is a scaffolded implementation repo for the coworld `diagnoser` role. A coworld bundles one game container, one or more player containers, and a `coworld_manifest.json`. The manifest can also declare optional roles, including `diagnoser`.

The diagnoser role exists in the schema today, but it does not have a documented runtime contract and is not launched by the runner.
...
For diagnosers, the top-level manifest field is:
...
Update this file when the metta repo gains a real diagnoser runtime contract, new role fields, new example coworlds, or runner/certifier behavior that changes how diagnosers are launched or validated.
