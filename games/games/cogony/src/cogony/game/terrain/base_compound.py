...
class BaseCompoundVariant(EnvNodeVariant[CompoundConfig]):
    ...
    def compat(self, mission: CogonyMission) -> bool:
        ...
        if isinstance(instance, RandomTransform.Config) and isinstance(instance.scene, Compound.Config):
            return True
        if isinstance(instance, Arena.Config):
            return True
        return False
    ...
    def extract_node(cls, env: MettaGridConfig) -> CompoundConfig:
        ...
        elif isinstance(instance, Arena.Config):
            return instance.hub

        raise TypeError("BaseCompoundVariant can only be applied to RandomTransform/Compound or Arena scenes")
