...
```
coworld_manifest.json Coworld manifest with config/results schemas
...
client/            Browser clients served by /client/player, /client/global, and /client/replay
...
Persephone's Escape includes a Coworld manifest and a separate tournament mode for the platform contract. Freeplay remains the default local mode. Tournament mode is selected automatically when `COGAME_CONFIG_URI` is set, or explicitly with:

```bash
npx tsx server.ts --mode=tournament
```

Tournament configs must include `"mode": "tournament"` and runner-managed `tokens`. The Coworld manifest is `coworld_manifest.json`. The upstream contract is documented in [Metta's Coworld spec](https://github.com/Metta-AI/metta/blob/main/packages/coworld/COWORLD_README.md).
