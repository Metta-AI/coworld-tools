...
```bash
docker build --platform=linux/amd64 -f Dockerfile -t coworld-amongcogs-game:latest .
docker build --platform=linux/amd64 -f player/Dockerfile -t coworld-amongcogs-player:latest .
```
