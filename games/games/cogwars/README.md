...
- a game registration available through `cogames.game.get_game("cogwars")` after import,
- a standalone `cogwars-play` CLI for quick local sanity checks,
...
cogwars-play --render none --max-steps 20

# 5. Stack variants
cogwars-play --render none --max-steps 10 -v easy -v big_map
```
...
`cogwars-play -v full` automatically pulls `hard` and `big_map` into the `VariantRegistry` through
...
1. `pyproject.toml` — `name`, `description`, URLs, and the script entry point
   `[project.scripts] cogwars-play = ...`.
2. `src/cogame/` → `src/<your_game>/` (update every import accordingly).
3. `cogame.game.MyCoGame.name = "cogwars"` → your game's registration name.
