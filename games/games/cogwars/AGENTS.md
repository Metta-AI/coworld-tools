...
cogwars-play --render none --max-steps 20      # standalone sanity check
cogwars-play -v easy                            # standalone CLI
```
...
- [`src/cogame/cli.py`](src/cogame/cli.py) — the `cogwars-play` console script.
...
1. **Run the code.** If a change is local and reversible, run `pytest`,
   `cogwars-play --render none`, or `cogames play` to verify. Don't ask
