"""Cogisis play-session artifacts and client links."""
...
class EpisodeArtifacts:
    ...
    @classmethod
    def create(cls, workspace: Path | str | None = None, *, prefix: str = "cogisis-play-") -> EpisodeArtifacts:
...
def build_play_links(tokens: list[str], *, game_port: int, base_url: str | None = None) -> PlayLinks:
    root = (base_url or f"http://127.0.0.1:{game_port}").rstrip("/")
    players = [
        f"{root}/player?{urlencode({'slot': slot, 'token': token})}"
        for slot, token in enumerate(tokens)
    ]
    return PlayLinks(
        players=players,
        global_=f"{root}/global",
        admin=f"{root}/admin",
    )
