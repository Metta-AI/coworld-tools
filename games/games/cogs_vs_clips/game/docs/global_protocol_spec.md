...
The HTTP `/client/global` client loads the bundled websocket-capable MettaScope from
...
`GET /client/replay?uri=<uri>` and sends the saved live-replay artifact over `WEBSOCKET /replay?uri=<uri>`.
