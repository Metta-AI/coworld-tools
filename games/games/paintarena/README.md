...
```bash
docker build --platform=linux/amd64 -f games/paintarena/Dockerfile -t coworld-paintarena:latest games/paintarena
```
...
```bash
uv run coworld play games/paintarena/coworld_manifest.json
```
...
```bash
uv run coworld run-episode games/paintarena/coworld_manifest.json coworld-paintarena:latest --run python --run -m --run coworld.examples.paintarena.player.player
```
...
```bash
uv run coworld certify games/paintarena/coworld_manifest.json
```
...
```bash
uv run coworld replay games/paintarena/coworld_manifest.json path/to/replay.json
```
