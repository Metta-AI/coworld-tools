...
def build_env(config: dict[str, Any]):
    ...
    _mission_name, env, _mission = resolve_mission(
        get_game("overcogged"),
        str(config.get("mission", "classic")),
        variants_arg=config.get("variants") or None,
        cogs=len(config["tokens"]),
    )
    env.game.max_steps = int(config["max_steps"])
    return env
