...
```bash
docker build --platform=linux/amd64 -f Dockerfile -t coworld-werecog-game:latest .
docker build --platform=linux/amd64 -f player/Dockerfile -t coworld-werecog-player:latest .
```
