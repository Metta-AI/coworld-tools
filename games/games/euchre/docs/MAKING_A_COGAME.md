...
```bash
euchre-play --render none --max-steps 200
```
...
For training, use the standalone CLI or Python API directly — there is no `cogames train` wrapper in this repo. [PufferLib](https://github.com/PufferAI/PufferLib) handles PPO with parallel vectorized environments. A trained checkpoint can be loaded back for play or eval:

```bash
euchre-play --policy file://./checkpoints/latest.pt
```

Before committing training time, run `euchre-play --render none --max-steps 200` to verify the game interactively — check that maps generate correctly, handlers fire, and rewards accumulate as expected.
