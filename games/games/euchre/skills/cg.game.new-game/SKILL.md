...
- **Recipe entrypoint:** `recipes/game/<name>.py` with both `play()` and `train()` functions. Without `play()`, the game
  ...
  `tools.PlayTool` (for play) or `tools.TrainTool` (for train). For standalone cogame repos, register via `register_game()`
  at the bottom of `src/<your_game>/game.py` and ensure the module is discovered via the side-effect import in `__init__.py`. The recipe MUST handle
...
- The target package surfaces for the build phase. Decide which package the game belongs in:
  - **`src/<your_game>/game.py`** for standalone cogame repos using the `CoGameMission`/`CoGame` framework
    (missions, variants, eval missions). Register via `register_game()` at the bottom of the game module AND ensure
    the side-effect import in `__init__.py` triggers it at startup. Without the init import, `get_game("<name>")`
  ...
  - **Standalone repo** for games developed outside the monorepo (e.g., tournament entries, external collaborators).
    These depend on `mettagrid`. The recipe still lives in the consuming repo's `recipes/` directory.
