...
tag-play --render none --max-steps 20      # standalone sanity check
tag-play -v easy                            # standalone CLI
```
...
- [`src/cogame/cli.py`](src/cogame/cli.py) — the `tag-play` console script.
...
1. **Run the code.** If a change is local and reversible, run `pytest`,
   `tag-play --render none`, or `cogames play` to verify. Don't ask
