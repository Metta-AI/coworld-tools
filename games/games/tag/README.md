...
- a game registration available through `cogames.game.get_game("tag")` after import,
- a standalone `tag-play` CLI for quick local sanity checks,
...
tag-play --render none --max-steps 20

# 5. Stack variants
tag-play --render none --max-steps 10 -v easy -v big_map
```
...
`tag-play -v full` automatically pulls `hard` and `big_map` into the `VariantRegistry` through
...
1. `pyproject.toml` — `name`, `description`, URLs, and the script entry point
   `[project.scripts] tag-play = ...`.
2. `src/cogame/` → `src/<your_game>/` (update every import accordingly).
3. `cogame.game.MyCoGame.name = "tag"` → your game's registration name.
