...
from diplomacog.coworld.server import build_env
...
def test_coworld_manifest_has_fixed_slots_and_reference_player() -> None:
    ...
    assert tokens["minItems"] == 6
    assert tokens["maxItems"] == 6
    assert len(manifest["certification"]["players"]) == 6
    assert manifest["player"][0]["image"] == "coworld-diplomacog-player:latest"


def test_coworld_build_env_uses_configured_slot_count() -> None:
    config = {"tokens": [f"token-{idx}" for idx in range(6)], "max_steps": 3, "seed": 0, "step_seconds": 0.01}
