...
```bash
docker build --platform=linux/amd64 -f Dockerfile -t coworld-diplomacog-game:latest .
docker build --platform=linux/amd64 -f player/Dockerfile -t coworld-diplomacog-player:latest .
```
