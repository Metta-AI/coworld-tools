...
coghouse-play --render none --max-steps 20      # standalone sanity check
```
...
- [`src/cogame/cli.py`](src/cogame/cli.py) — the `coghouse-play` console script.
...
1. **Run the code.** If a change is local and reversible, run `pytest` or
   `coghouse-play --render none` to verify. Don't ask permission for local,
