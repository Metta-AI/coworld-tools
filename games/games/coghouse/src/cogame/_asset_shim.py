...
mettascope data and overlays cogame's custom sprites on top. Since
...
def _build_merged_data_dir() -> Path:
    """Create a temp dir that merges mettagrid's data with cogame sprites."""
    ...
    merged = Path(tempfile.mkdtemp(prefix="cogame_data_"))
...
class CogameRenderer(MettascopeRenderer):
    """MettascopeRenderer that includes cogame sprites in the atlas."""
