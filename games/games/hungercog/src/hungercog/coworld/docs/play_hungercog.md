...
```bash
docker build --platform=linux/amd64 -f Dockerfile -t coworld-hungercog-game:latest .
docker build --platform=linux/amd64 -f player/Dockerfile -t coworld-hungercog-player:latest .
```
